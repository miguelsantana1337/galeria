...
const schema = z.object({ key: z.string().uuid(), photoIds: z.array(z.string().uuid()).min(1).max(100) });
export async function POST(request: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  const admin = createSupabaseAdminClient(); const { data: event } = await admin.from("events").select("id,expires_at").eq("slug", slug).eq("share_token", parsed.data.key).eq("status", "published").single(); if (!event || (event.expires_at && new Date(event.expires_at) <= new Date())) return NextResponse.json({ error: "Galeria indisponível." }, { status: 404 });
  ...
  return NextResponse.json({ photos: signed.filter((photo) => photo.url) }, { headers: { "Cache-Control": "private, no-store" } });
}
